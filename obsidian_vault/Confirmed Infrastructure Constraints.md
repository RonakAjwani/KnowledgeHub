...
- ✅ **Use native weighted RRF — but the server floor is 1.16+, not 1.14.** ⚠️ Corrected empirically 2026-07-28, against a running container. The settable-`k` query object — `RrfQuery(rrf=Rrf(k=…, weights=[…]))`, the only form where both terms of `RRF_MAX` are our own constants — is **rejected outright by server 1.15.1** (`Format error in JSON body: Expected some form of vector, id, or a type of query`). `FusionQuery(fusion=Fusion.RRF)` *is* accepted on 1.15 but carries neither `weights` nor `k`, so it cannot support a trustworthy `RRF_MAX` and would make every G2 threshold silently meaningless. **Compose is pinned to `v1.18.0`.** Details in [[Technology Documentation Links]] → Findings #9.
