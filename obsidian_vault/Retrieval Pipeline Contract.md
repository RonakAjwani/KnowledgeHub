...
| `answer.delta` | `{text}` | Token chunk. Only between `pipeline.stage{generate,started}` and a terminal event. **This is the answer's only transport** - `answer.complete` carries no text, so a terminal node that builds its answer whole (`refuse`, `history`) must still send it as one delta inside a `generate` bracket. Emitting `answer.complete` alone persisted a real answer while the client, which concatenates deltas, rendered an empty bubble (measured 2026-08-02) |
