...
### API findings - 2026-08-02, post-recording bug sweep

17. ⚠️ **Clerk's `appearance.elements` table in the docs is not the full descriptor list, and the "Last used" pill is one of the entries missing from it.** The badge Clerk renders on whichever sign-in option the visitor used last is `lastAuthenticationStrategyBadge` (localisation key `lastAuthenticationStrategy: "Last used"`). It is absent from the published elements page; the authoritative list is the descriptor array inside the loaded `@clerk/ui` bundle itself, next to `socialButtonsBlockButton` / `socialButtonsProviderInitialIcon`. **Read the bundle, not the docs table, when an element has no documented key** - clerk-js is served from `<instance>.clerk.accounts.dev/npm/@clerk/...`, not from `node_modules`, so the installed version's own JS is the only ground truth. Rendering it also requires `client.lastAuthenticationStrategy`, which is server state from a real prior sign-in; rewriting the FAPI `/v1/client` response is how it was made visible without one.
18. ⚠️ **Clerk's `spacing` variable scales its box metrics but not its type**, so lowering it makes any element Clerk sizes to fit its own text too short for that text. `spacing: 0.7rem` (set so the sign-up card fits AuthShell without scrolling) gave `lastAuthenticationStrategyBadge` a hard `height: 14.17px` and `display: block` around a 15px line box - `scrollHeight` 18 vs `clientHeight` 12, label hanging 2.8px below the pill. Fixed per-element with `h-auto` + `inline-flex items-center` rather than by raising `spacing`. Worth checking any other Clerk element with no vertical slack.
19. **Resolved frontend auth versions (2026-08-02):** `@clerk/nextjs` **7.6.1**, `@clerk/shared` 4.25.8, and at runtime `clerk-js` **6.25.13** with `@clerk/ui` **1.27.2**. Note the runtime pair is *not* pinned by `pnpm-lock.yaml` - Clerk serves it from its CDN and can move it underneath a fixed `@clerk/nextjs`, which is exactly how an element descriptor can appear or change without a dependency bump.

20. ✅ **Cohere's `relevance_score` must be carried through, not recomputed from position.** `_reorder` originally assigned `1.0 - position/len(order)`, which made `0.6·max + 0.4·mean` a fixed function of `top_n`: exactly **0.840 on every query** at `top_n = 5`, regardless of whether anything relevant was retrieved. `FLOOR_RERANK` was a comparison against a constant and could never fire - the same failure mode I7 forbids, arrived at from a different direction. The cache must store `(id, score)` pairs for the same reason.
